    ...
    def forward(self, feature, target, score_map=None, domain='source', alpha=0.0, labels=None, reg_targets=None, conf_th=0.5):
        ...
        if self.expand:
            ...
            if self.reg_left_align or self.reg_top_align:
                ...
                box_cls_gt = torch.log2(box_regression_map.unsqueeze(-1))
                if self.reg_left_align:
                    feature_['reg_l'] = feature * box_cls_gt[:, 0]
                    feature_['reg_l'] = feature_['reg_l'].reshape(sh[0], sh[2], sh[3], -1).permute(0,3,1,2)
                    feature_['reg_l'] = feature_['reg_l'] * atten_map
                if self.reg_top_align:
                    feature_['reg_t'] = feature * box_cls_gt[:, 1]
                    feature_['reg_t'] = feature_['reg_t'].reshape(sh[0], sh[2], sh[3], -1).permute(0,3,1,2)
