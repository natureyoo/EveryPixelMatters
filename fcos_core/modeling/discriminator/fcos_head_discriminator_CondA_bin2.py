...
class FCOSDiscriminator_CondA(nn.Module):
    def __init__(self, num_convs=2, in_channels=256, grad_reverse_lambda=-1.0, center_aware_weight=0.0, \
        ...
        dis_tower = []
        self.num = {'class':8, 'reg_l':2, 'reg_t':2, 'reg_b':2, 'reg_r':2}
        ...
        if level == 'P3':
            ...
            self.bin_mean = torch.tensor([3.5,4.5]).cuda()
        elif level == 'P4':
            # self.bin_mean = torch.tensor([12,20,44]).cuda()
            # self.bin_mean = torch.tensor([20,44,84]).cuda()
            self.bin_mean = torch.tensor([4.5,5.5]).cuda()
        elif level == 'P5':
            # self.bin_mean = torch.tensor([20,44,84]).cuda()
            # self.bin_mean = torch.tensor([44,84,172]).cuda()
            self.bin_mean = torch.tensor([5.5,6.5]).cuda()
        elif level == 'P6':
            # self.bin_mean = torch.tensor([44,84,172]).cuda()
            # self.bin_mean = torch.tensor([84,172,340]).cuda()
            self.bin_mean = torch.tensor([6.5,7.5]).cuda()
        else:
            # self.bin_mean = torch.tensor([44,84,172]).cuda()
            # self.bin_mean = torch.tensor([84,172,340]).cuda()
            self.bin_mean = torch.tensor([7.5,8.5]).cuda()
        # self.bin_std = self.bin_mean / 4
        self.bin_std = torch.tensor([0.1,0.1]).cuda()
        self.tau = 0.1
        self.stat = {'source': [1/2, 1/2], 'target': [1/2, 1/2]}
        if expand_dim > 0:
            self.expand = True
        ...
        if self.expand:
            ...
            if self.reg_left_align or self.reg_top_align:
                ...
                if self.reg_left_align:
                    ...
                    box_cls_onehot = alpha * torch.ones(box_cls_onehot.shape).cuda() / 2 + (1 - alpha) * box_cls_onehot
